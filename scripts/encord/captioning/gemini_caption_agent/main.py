...
def echo_auth_context(config: AgentConfig, config_path: Path) -> None:
    ...
    typer.echo(
        f"[auth] {config.gemini_api_key_env_var}: {secret_env_status(config.gemini_api_key_env_var)}"
    )
...
def runner_or_exit(config: AgentConfig) -> Runner:
    ...
    try:
        ...
        typer.echo(f"[auth] could not initialize Encord task Runner: {exc}", err=True)
...
def metadata_sources_for(
    storage_item: StorageItem, label_row: LabelRowV2 | None = None
) -> list[dict[str, Any]]:
...
def find_task_name(
    metadata_sources: Iterable[dict[str, Any]], config: AgentConfig
) -> str | None:
...
def is_video_storage_item(storage_item: StorageItem) -> bool:
    ...
    return any(
        "observation.images." in str(metadata.get(key, ""))
        for key in ("source_uri", "s3_uri", "objectUrl")
    )
...
def select_videos(
    ...
    video_children = [
        child for child in storage_children if is_video_storage_item(child)
    ]
    if not video_children:
        available = sorted(layout_to_file_name) or [
            str(getattr(child, "name", "")) for child in storage_children
        ]
        raise RuntimeError(
            f"No video child found. Wanted layout {config.video_layout!r}; available {available}"
        )
...
@contextmanager
def temporary_video_paths(
    selected_videos: list[SelectedVideo],
) -> Iterator[list[tuple[SelectedVideo, Path]]]:
    """Download selected assets for one task and remove them when the task finishes."""
    with ExitStack() as assets:
        yield [
            (video, assets.enter_context(download_asset(video.storage_item)))
...
def wait_until_ready(
    ...
    while True:
        ...
        state = getattr(getattr(file_obj, "state", None), "name", None) or str(
            getattr(file_obj, "state", "UNKNOWN")
        )
        if state == "ACTIVE":
            return file_obj
        if state == "FAILED":
        ...
        if time.monotonic() >= deadline:
            raise TimeoutError(
                f"Timed out waiting for Gemini file {file_name}; last state={state}"
            )
        time.sleep(2)
...
def run_gemini_caption(
    *,
    ...
    try:
        ...
        ready_files = [
            wait_until_ready(
                client, uploaded.name, config.file_processing_timeout_seconds
            )
        ...
        if not text:
            raise ValueError(
                f"Gemini returned no text. {gemini_response_summary(response)}"
            )
        try:
...
def validate_captions(result: GeminiCaptionResponse) -> None:
    ...
    if missing:
        raise ValueError(
            f"Gemini returned empty language instruction value(s): {missing}"
        )
    ...
    if not normalized[2].startswith("use the robot arm to "):
        raise ValueError(
            'Language Instruction 3 must start with "use the robot arm to ".'
        )
    ...
    for caption in captions:
        ...
        for pattern in BAD_CAPTION_PATTERNS:
            if re.search(pattern, lowered):
                raise ValueError(
                    f"Rejected caption {caption!r}; matched banned pattern {pattern!r}."
                )
...
def ensure_label_row_initialized(label_row: LabelRowV2) -> None:
    try:
        ...
        if (
            "already been initialized" in message
            or "already been initialised" in message
        ):
            return
        raise
...
def existing_caption_answers(
    label_row: LabelRowV2, config: AgentConfig
) -> dict[str, str]:
...
def validate_project_setup(runner: Runner, config: AgentConfig) -> None:
    project = getattr(runner, "project", None) or runner.client.get_project(
        config.project_hash
    )
    failures: list[str] = []
    ...
    except Exception as exc:  # noqa: BLE001 - report all project contract failures together
        stage_titles = [
            getattr(stage, "title", "<unknown>") for stage in project.workflow.stages
        ]
        failures.append(
    ...
    for title in config.caption_titles:
        try:
            project.ontology_structure.get_child_by_title(
                title=title, type_=Classification
            )
        except Exception as exc:  # noqa: BLE001 - SDK lookup failures vary by version
            failures.append(f"missing classification {title!r}: {exc!r}")

    try:
        gemini_client(config)
    except Exception as exc:  # noqa: BLE001 - surface missing or invalid Gemini credentials
    ...
    if sample_task is None:
        typer.echo(
            "[check] no pending task at the agent stage; skipping sample video-layout check"
        )
        return
    ...
    typer.echo(
        f"[check] selected video: {', '.join(item.layout_key for item in selected)}"
    )
    ...
    def caption_with_gemini(
        ...
        try:
            ...
            if len(existing) == len(config.caption_titles) and not config.overwrite:
                typer.echo(
                    f"[agent] {task.data_hash}: all captions already present; skipping"
                )
                return config.success_pathway

            selected = select_videos(label_row, storage_item, config)
            task_name = find_task_name(
                metadata_sources_for(storage_item, label_row), config
            )
            if not task_name:
                raise RuntimeError(
            ...
            with temporary_video_paths(selected) as video_paths:
            ...
            for video in selected:
                if video.fallback:
                    typer.echo(
                        f"[agent] layout fallback: {video.fallback_reason}; selected {video.layout_key}"
                    )
            return route
        except errors.APIError as exc:
            typer.echo(f"[agent] {task.data_hash}: Gemini API failed: {exc}", err=True)
            return config.failure_pathway
        except Exception as exc:  # noqa: BLE001 - every task failure must take a pathway
...
def debug_auth(
    ...
    typer.echo(
        f"[auth] Encord project access OK: {project.title} ({config.project_hash})"
    )
