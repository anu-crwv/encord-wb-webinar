...
# ]
# ///
"""Update the Encord client metadata schema from a registration JSON.
...
def update_schema(client: EncordUserClient, enum_values: dict[str, set[str]], dry_run: bool) -> None:
    ...
    for change in changes:
        typer.echo(change)
    if not changes:
        typer.echo("Metadata schema already up to date.")
    if not dry_run:
        schema.save()
        typer.echo("Saved updated metadata schema.")
...
def main(
    ...
    update_schema(client, enum_values, dry_run=dry_run)
